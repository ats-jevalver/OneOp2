...
async function loadAccountPlan() {
  ...
  try {
    ...
    return `<div class="card full"><h3>Account Plan</h3><p><strong>${esc(plan.planName)}</strong> ${badge(plan.status)}</p><p>${esc(plan.planSummary)}</p><p class="muted small">Owner: ${esc(plan.owner?.displayName || 'Unassigned')} | Review: ${esc(plan.targetReviewDate || 'N/A')}</p><h4>Objectives</h4><ul class="clean">${plan.objectives.map(o => `<li>${badge(o.priority)} ${esc(o.title)} <span class="muted small">${esc(o.status)} | ${esc(o.successMetric || '')}</span></li>`).join('')}</ul><h4>Stakeholders</h4><ul class="clean">${plan.stakeholders.map(s => `<li><strong>${esc(s.contact?.fullName || s.contactId)}</strong> - ${esc(s.stakeholderRole)} <span class="muted small">${esc(s.relationshipStrength || '')} ${esc(s.sentiment || '')}</span></li>`).join('')}</ul><p><button class="secondary" id="refreshPlanSummary">Mark Plan Reviewed</button></p></div>`;
  } catch {
    return '';
  }
}
