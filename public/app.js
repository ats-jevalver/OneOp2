...
async function loadSystemStatus() {
  try {
    ...
    const authMode = user.auth?.authProvider || user.auth?.authMode || 'unknown';
    const localDemoWarning = user.auth?.unsafeForProduction ? '<p class="warning small">Local demo user switching is unsafe for production and is shown for pilot testing only.</p>' : '';
    systemStatus.innerHTML = `<div><strong>Provider:</strong> ${badge(provider)} <span class="muted">${active}</span></div><div><strong>Auth:</strong> ${badge(authMode)} <span class="muted">${user.auth?.localDemoOnly ? 'local demo only' : 'production provider'}</span></div><div><strong>User:</strong> ${esc(user.displayName)} <span class="muted">${esc(user.role)}</span></div>${localDemoWarning}<p><button class="secondary" id="beAdmin">Use Admin Demo User</button> <button class="secondary" id="beManager">Use Account Manager</button> <button class="secondary" id="refreshStatus">Refresh</button></p><div id="databaseStatus" class="muted small">Admin database status is available after switching to admin.</div>`;
...
async function generateArtifact(kind) {
  ...
  const canEmail = artifact.artifactType === 'customer_email_draft';
  briefBody.innerHTML = `<p><strong>${artifact.title}</strong> ${badge(artifact.status)}</p><textarea class="brief-text" readonly>${artifact.body}</textarea><p><a href="${exportLink}" target="_blank" rel="noreferrer">Open markdown export</a> <button id="exportArtifactFile">Export Markdown File</button>${canEmail ? ` <button id="emailHandoff">Prepare Email Handoff</button>` : ''}</p><p class="muted small">Sprint 8 exports include an evidence appendix. Draft artifacts may show review warnings until marked reviewed or approved.</p><div id="artifactActionResult" class="muted small"></div>`;
  briefDialog.showModal();
  document.getElementById('exportArtifactFile').onclick = async () => {
    const result = await api(`/api/v1/generated-artifacts/${artifact.generatedArtifactId}/export-file?format=markdown`, { method: 'POST' });
    document.getElementById('artifactActionResult').innerHTML = `<strong>Exported:</strong> ${esc(result.relativePath)} (${result.bytesWritten} bytes)${result.warnings?.length ? `<br><span class="warning">${esc(result.warnings.join(' '))}</span>` : ''}`;
  };
  const handoff = document.getElementById('emailHandoff');
  if (handoff) handoff.onclick = async () => {
    const result = await api(`/api/v1/generated-artifacts/${artifact.generatedArtifactId}/email-handoff`, { method: 'POST' });
    document.getElementById('artifactActionResult').innerHTML = `<strong>Email handoff:</strong> ${badge(result.status)} ${esc(result.prepareEmail.Subject)}<br><span class="muted">To: ${(result.prepareEmail.To || []).map(esc).join(', ') || 'No primary contact'}</span>${result.warnings?.length ? `<br><span class="warning">${esc(result.warnings.join(' '))}</span>` : ''}<br><span class="muted">No send occurs from OneOp2.</span>`;
  };
...
async function createTaskFlow(recommendationId) { const preview = await api(`/api/v1/accounts/${currentAccountId}/psa/tasks/preview`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ recommendationId }) }); taskBody.innerHTML = `<label>Title<input id="taskTitle" value="${esc(preview.payload.title)}"></label><label>Body<textarea id="taskText">${esc(preview.payload.body)}</textarea></label><p class="warning">${preview.warnings.join(' ')}</p><p class="muted small">Evidence: ${preview.payload.evidenceSummary.join(' | ')}</p><p class="muted small">Preview fingerprint: ${esc(preview.previewFingerprint)}</p><button id="confirmTask">Confirm Stub Task</button>`; taskDialog.showModal(); document.getElementById('confirmTask').onclick = async () => { const created = await api(`/api/v1/accounts/${currentAccountId}/psa/tasks`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ recommendationId, title: document.getElementById('taskTitle').value, body: document.getElementById('taskText').value, previewFingerprint: preview.previewFingerprint, confirmed: true }) }); taskBody.innerHTML = `<p>Created stub PSA task: <strong>${created.externalId}</strong></p><p class="muted">${created.externalUrl}</p>`; await loadAccount(currentAccountId); }; }
...
async function showAdminIntegrations() {
  ...
  try {
    const [status, config, diagnostics, history] = await Promise.all([
      api('/api/v1/integrations/status'),
      api('/api/v1/admin/integrations/int_psa_demo/configuration'),
      api('/api/v1/admin/integrations/int_psa_demo/diagnostics'),
    ...
    commandCenter.innerHTML = `<h2>Admin Integrations</h2><div class="cards"><div class="card"><h3>Provider</h3><p><strong>Store:</strong> ${badge(status.store.provider)} ${esc(status.store.message || '')}</p><p><strong>PSA:</strong> ${esc(psa?.systemName || 'Demo PSA')} ${badge(psa?.status || 'unknown')}</p><p class="muted small">Capabilities: ${(psa?.capabilities || []).map(esc).join(', ')}</p></div><div class="card"><h3>Connector Diagnostics</h3><p><strong>Mode:</strong> ${badge(diagnostics.adapterMode)} <strong>Provider:</strong> ${badge(diagnostics.providerType)}</p><p><strong>Status:</strong> ${badge(diagnostics.status)}</p><p class="muted small">Config complete: ${diagnostics.config?.complete ? 'yes' : 'no'} | Secrets configured: ${diagnostics.secrets?.configured ? 'yes' : 'no'} | External mutation: ${diagnostics.source?.externalMutationAllowed ? 'enabled' : 'disabled'}</p><p class="muted small">${esc(diagnostics.message || '')}</p></div><div class="card"><h3>PSA Configuration</h3><label>Environment<input id="psaEnvironment" value="${esc(config.environmentLabel || '')}"></label><label>Base URL<input id="psaBaseUrl" value="${esc(config.baseUrl || '')}"></label><label>Tenant/Company<input id="psaTenant" value="${esc(config.tenantOrCompanyId || '')}"></label><p class="muted small">Secret status: ${esc(config.secretStatus || 'not_configured')}. Secrets are never returned by this UI.</p><button id="savePsaConfig">Save Non-Secret Config</button></div><div class="card full"><h3>Read-only PSA Validation</h3><p><button id="validatePsaReads">Validate Company / Contact / Ticket Reads</button></p><div id="psaReadValidationResult" class="muted">Run validation to inspect adapter read shape, source mode, and sample rows without external mutation.</div></div><div class="card full"><h3>Sync Preview / Apply</h3><p><button id="previewPsaSync">Preview PSA Company/Contact Sync</button> <button class="secondary" id="applySafePsaRows" disabled>Apply Safe Selected Rows</button></p><div id="syncPreviewResult" class="muted">Run preview to inspect rows before applying.</div></div><div class="card full"><h3>Sync History</h3><ul class="clean" id="syncHistoryList">${history.length ? history.map(run => `<li><strong>${esc(run.status)}</strong> ${badge(run.adapterMode || 'unknown')}: applied ${run.counts.applied}, conflicts ${run.counts.conflicts} <span class="muted small">${esc(run.appliedAt)}</span></li>`).join('') : '<li>No Sprint 8 sync apply runs yet.</li>'}</ul></div></div>`;
    ...
    document.getElementById('validatePsaReads').onclick = async () => {
      const [companies, contacts, tickets] = await Promise.all([
        api('/api/v1/admin/integrations/int_psa_demo/psa/companies?search=acme'),
        api('/api/v1/admin/integrations/int_psa_demo/psa/contacts?externalCompanyId=PSA-1001'),
        api('/api/v1/admin/integrations/int_psa_demo/psa/tickets?status=open')
      ]);
      const sample = row => esc(row.displayName || row.fullName || row.title || row.externalCompanyId || row.externalTicketId || 'row');
      const block = payload => `<div class="result"><strong>${esc(payload.recordType)}</strong> ${badge(payload.adapterMode || 'unknown')} <span class="muted small">${payload.counts.total} row(s), external mutation ${payload.source?.externalMutationAllowed ? 'enabled' : 'disabled'}</span><br><span class="muted small">${esc((payload.rows || []).slice(0, 3).map(sample).join(', ') || (payload.warnings || []).join(' ') || 'No rows returned.')}</span></div>`;
      document.getElementById('psaReadValidationResult').innerHTML = `<p><strong>Provider:</strong> ${badge(companies.providerType || 'unknown')} <strong>Mode:</strong> ${badge(companies.adapterMode || 'unknown')} <span class="muted small">Secrets returned: ${companies.secretsReturned ? 'yes' : 'no'}</span></p>${[companies, contacts, tickets].map(block).join('')}`;
    };
    document.getElementById('previewPsaSync').onclick = async () => { const preview = await api('/api/v1/admin/integrations/int_psa_demo/sync-preview', { method: 'POST' }); const safeRows = [...preview.companies, ...preview.contacts].filter(row => row.action !== 'conflict').slice(0, 3).map(row => row.rowId); document.getElementById('applySafePsaRows').dataset.rows = safeRows.join(','); document.getElementById('applySafePsaRows').disabled = false; document.getElementById('syncPreviewResult').innerHTML = `<p><strong>Mode:</strong> ${badge(preview.adapterMode || 'unknown')} <strong>Total:</strong> ${preview.counts.total} | New: ${preview.counts.new} | Matched: ${preview.counts.matched} | Changed: ${preview.counts.changed} | Conflicts: ${preview.counts.conflicts}</p><p class="muted small">External mutation: ${preview.source?.externalMutationAllowed ? 'enabled' : 'disabled'} | ${esc((preview.warnings || []).join(' '))}</p><ul class="clean">${preview.companies.map(row => `<li>${badge(row.action)} ${esc(row.displayName)} <span class="muted small">${esc(row.reason)}</span></li>`).join('')}</ul>`; };
